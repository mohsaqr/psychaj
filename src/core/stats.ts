...
/**
 * Student's t quantile (inverse CDF): returns t such that P(T <= t) = p,
 * T ~ t(df). Bisection on tDistCDF; converges to ~1e-9 in 64 iterations.
 */
export function tDistInv(p: number, df: number): number {
...
/**
 * Percentile with linear interpolation (matches R quantile type=7).
 */
 ...
 */
export function pearsonCorr(a: Float64Array | number[], b: Float64Array | number[]): number {
  const n = a.length;
  if (n < 2) return 0;
  let sumA = 0, sumB = 0;
